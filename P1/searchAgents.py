...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.startState = (startingGameState.getPacmanPosition(), set())
    ...
    def getSuccessors(self, received_state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                newVisited = received_state[1].copy()
...
def cornersHeuristic(cur_state, problem):
    ...
    for c in corners:
        if c not in visitedCorners:
            dist = sqrt((c[0] - node[0])**2 + (c[1] - node[1])**2)

            auxVisited = visitedCorners.copy()
            auxVisited.add(c)

            while len(auxVisited) < len(corners):
                minDist = 0
                minCorner = None

                for c2 in corners:
                    if c2 not in auxVisited:
                        dist2 = sqrt((c2[0] - c[0])**2 + (c2[1] - c[1])**2)

                        if minCorner == None or dist2 < minDist:
                            minDist = dist2
                            minCorner = c2

                dist += minDist
                auxVisited.add(minCorner)
                c = minCorner

            if dist < minHeur or minHeur == 0:
                minHeur = dist
